(function () {
    window.Util = {
        allCheck: function (id, callback){
            const _this = $('#' + id);
            const ul = _this.find('.modal-body>ul');

            if (ul.find('li').length > ul.find('li>input:checked').length) {
                ul.find('li>input').attr('checked', true);
                ul.find('li>input').prop('checked', true);
            } else {
    ...
    window.MenuUtil = {
        move: function (url, menuId) {
            location.href = url.includes('?') ? `${url}&menuId=${menuId}` : `${url}?menuId=${menuId}`;
        },
        getMenuId: function () {
            return window.commonData?.menuId;
        },
    };
    ...
    window.callModule = {
        ...
        call: function (url, data, callback, async, method) {
            ...
            $.ajax({
                beforeSend: function (xhr) {
                    ...
                    if (MenuUtil.getMenuId()) {
                        xhr.setRequestHeader("MENUID", MenuUtil.getMenuId());
                    }
                },
                cache: false,
                async: async,
        ...
        callForm: function (frmId, callback, async) {
        ...
        successHandler: function (result, callback) {
            ...
            } else if (result.defaultVO && result.defaultVO.successMessage != undefined && result.defaultVO.successMessage.indexOf("조회") == -1) {
                MessageUtil.success(result.defaultVO.successMessage, callback, result);
            } else if (result.defaultVO && result.defaultVO.errorMessage != undefined) {
                MessageUtil.error(result.defaultVO.errorMessage, callback);
            } else {
                if (callback && callback instanceof Function) {
                    callback(result);
                }
        ...
        loadHtml: function (element, url, callback) {
            $.ajax({
                url: url,
                dataType: 'html',
                async: false,
        ...
        showLoading: function (isBlind = false) {
            try {
                $("#loader-list").show();
                if (isBlind) {
                    $("#loader-list").addClass('blind');
                }
            } catch (error) {
        ...
        hideLoading: function () {
            try {
                $("#loader-list").hide().removeClass('blind');
            } catch (error) {
